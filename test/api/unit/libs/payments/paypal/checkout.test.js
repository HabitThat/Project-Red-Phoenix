...
describe('paypal - checkout', () => {
  ...
  it('creates a link for gem purchases', async () => {
    ...
    expect(paypalPaymentCreateStub).to.be.calledWith(getPaypalCreateOptions('Accomplay Gems', 4.99));
  ...
  it('creates a link for gifting gems', async () => {
    ...
    expect(paypalPaymentCreateStub).to.be.calledOnce;
    expect(paypalPaymentCreateStub).to.be.calledWith(getPaypalCreateOptions('Accomplay Gems (Gift)', '4.00'));
    expect(link).to.eql(approvalHerf);
  });
  ...
  it('creates a link for gifting a subscription', async () => {
    ...
    expect(paypalPaymentCreateStub).to.be.calledOnce;
    expect(paypalPaymentCreateStub).to.be.calledWith(getPaypalCreateOptions('mo. Accomplay Subscription (Gift)', '15.00'));
    expect(link).to.eql(approvalHerf);
  });
});
