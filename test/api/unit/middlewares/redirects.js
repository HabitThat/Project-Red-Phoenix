...
describe('redirects middleware', () => {
  ...
  context('forceSSL', () => {
    it('sends http requests to https', () => {
      ...
      nconfStub.withArgs('BASE_URL').returns('https://accomplay.com');
      ...
      expect(res.redirect).to.be.calledWith('https://accomplay.com/static/front');
    });
    ...
    it('does not redirect if base URL is not https', () => {
      const nconfStub = sandbox.stub(nconf, 'get');
      nconfStub.withArgs('BASE_URL').returns('http://accomplay.com');
      nconfStub.withArgs('IS_PROD').returns(true);
      req.protocol = 'http';
      req.originalUrl = '/static/front';
    ...
    it('does redirect if skip ssl request param is passed with incorrrect key', () => {
      ...
      expect(res.redirect).to.be.calledOnce;
      expect(res.redirect).to.be.calledWith('https://accomplay.com/static/front?skipSSLCheck=INVALID');
    });
  ...
  context('forceAccomplay', () => {
    it('sends requests with differing hostname to base URL host', () => {
      ...
      attachRedirects.forceAccomplay(req, res, next);

      expect(res.redirect).to.be.calledOnce;
      expect(res.redirect).to.be.calledWith(301, 'https://accomplay.com/static/front');
    });
